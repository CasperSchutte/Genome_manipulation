...
class GenomeMutation:
    ...
    def Inversion(self, template_sequence, start_position, end_position):
        ...
        inverted_fragment = list(fragment)
        new_list=[]
        for x in inverted_fragment:
            if x > 0:

                new_list.append(-1*x)
            else:
                new_list.append(x)
        inverted_fragment = new_list
        print(inverted_fragment)
        #new_sequence.replace(fragment,inverted_fragment)
        new_sequence = list(new_sequence[0:start_position-1]) + inverted_fragment + list(new_sequence[end_position:])
